...
func getGenDepsCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:          "gen-deps",
		Short:        "Generates a go.mod file for the given plugin module",
		SilenceUsage: true,
...
func (r *moduleResolver) getModName(model string) string {
	return fmt.Sprintf("github.com/onosproject/onos-p4-plugins/plugins/%s", model)
}
...
func (r *moduleResolver) fetchMod(target string) (*modfile.File, error) {
	...
	fakeModDir, err := ioutil.TempDir("", "onos-p4-plugin")
	if err != nil {
		return nil, err
	}
